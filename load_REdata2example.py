...
SET_TYPES = [TRAIN_SET, DEV_SET, TEST_SET, ALL_SET, FEW_SHOT_SET]
...
def load_examples_from_ori_file(dataset_name, data_dir_parent: str, set_type: str, num_examples: int = None,
    ...
    if set_type == ALL_SET:
        ...
        all_examples = []
        all_examples.extend(processor.get_dev_examples(data_dir)[0]) # 使用append()导致：[[], [], []],不是把元素直接合并为同一个list
        all_examples.extend(processor.get_test_examples(data_dir)[0])
        all_examples.extend(processor.get_train_examples(data_dir)[0])
        ...
        if ((num_examples is not None) and (num_examples > 0)):
            examples = _shuffle_and_restrict(processor.get_dev_examples(data_dir), num_examples, seed)
        ...
        if ((num_examples is not None) and (num_examples > 0)):
            examples = _shuffle_and_restrict(processor.get_train_examples(data_dir), num_examples, seed)
        ...
        if ((num_examples is not None) and (num_examples > 0)):
            examples = _shuffle_and_restrict(processor.get_test_examples(data_dir), num_examples, seed)
...
def load_examples_from_new_file():
    ...
    #     assert(train_set), "train-set is not passed in load_examples function "
    #     assert(dev_set), "dev_set is not passed in load_examples function "
    #     vis = []
    #     for ex in train_set:
    #         vis.append(ex.idx)
    #     for ex in processor.get_train_examples(data_dir):
    #         if ex.idx in vis:
    #             continue
    #         ex.idx = len(examples) - 1
    #         examples.append(ex)
    ...
    # if set_type == ALL_SET or set_type == TEST_SET:
    #     num_examples = len(examples)
    
    # examples = _shuffle_and_restrict(examples, num_examples, seed) # 这之后就不再shuffle了，保持该顺序
