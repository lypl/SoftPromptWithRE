...
def get_entity_type(entity_tokens_type: List, relation_type: List, flag: str) -> str:
    """ relation_type: ["PERSON:PERSON", "PERSON:MISC"]
        entity_tokens_type: [(token, type_from_NER), (), ...]
    """
    # print("==== in get_entity_type() ====")
    # NA no_relation
    if relation_type == None or len(relation_type) == 0:
        ret = None
        if flag == "subj":
            per_cnt = 0
            org_cnt = 0
            for ty in entity_tokens_type:
                if ty == "PERSON":
                    per_cnt += 1
    ...
    for ty in relation_type:
        ...
        subj_ty = ty[:pos]
        obj_ty = ty[pos+1:]
        if subj_ty not in rel_subj_type:
            rel_subj_type.append(subj_ty)
        if obj_ty not in rel_obj_type:
            rel_obj_type.append(obj_ty)
    
    rel_type = None
    if flag == "subj":
        rel_type = rel_subj_type
    else:
        # print(entity_tokens_type)
        rel_type = rel_obj_type
    assert(rel_type is not None and len(rel_type) >= 1)
    
    if len(rel_type) == 1:
        return rel_type[0]
    else:
        entity_type_cnt = {}
        for it in entity_tokens_type:
            st = it[0]
            ed = it[1]
            if ed not in entity_type_cnt.keys():
                entity_type_cnt[ed] = 1
            else:
                entity_type_cnt[ed] += 1
        candidate_type = []
        for ty in rel_type:
            if ty not in entity_type_cnt.keys():
                continue
            candidate_type.append((ty, entity_type_cnt[ty]))
        
        if len(candidate_type) == 0:
            idx = random.randint(0, len(rel_obj_type)-1)
            candidate_type.append((rel_obj_type[idx], 1))
        # assert(len(candidate_type) > 0)
        sorted(candidate_type, key=lambda x: x[1])
        candidate_type = candidate_type[-1:]
        # print(candidate_type)
        return candidate_type[0][0] # 只返回type，不返回个数
